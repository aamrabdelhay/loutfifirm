...
export function AssistantWidget() {
  ...
  async function sendMessage(value = input) {
    ...
    try {
      ...
      setMessages((current) => [...current, { role: "assistant", content: data.answer || t.greeting }]);
    } catch {
      ...
      {open ? (
        <section className="assistant-window" aria-label={t.title}>
          ...
          <div className="assistant-messages">
            {visibleMessages.map((m, i) => <div key={`${m.role}-${i}`} className={m.role === "user" ? "assistant-bubble user" : "assistant-bubble bot"}>{m.content}</div>)}

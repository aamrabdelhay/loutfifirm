...
type AssistantSettings = { configured: boolean; source: "environment" | "admin" | "none"; maskedKey: string; model: string; provider?: string };
...
export function AdminApp() {
  ...
  const [assistantModel, setAssistantModel] = useState("gemini-3.6-flash");
  ...
  const loadAssistant = useCallback(async () => {
    try {
      ...
      if (d.ok) { setAssistant(d); setAssistantModel(d.model || "gemini-3.6-flash"); }
  ...
  async function saveAssistant() {
    ...
    try {
      const res = await adminFetch("/api/admin/assistant-settings", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ apiKey: assistantKey, model: assistantModel }) });
      const d = await res.json(); if (!res.ok || !d.ok) throw new Error();
      setAssistant({ configured: true, source: "admin", maskedKey: d.maskedKey, model: d.model, provider: "google-gemini" });
      setAssistantKey(""); setAssistantEditing(false); setAssistantMsg("تم حفظ إعدادات Gemini بنجاح. تم إخفاء المفتاح من الواجهة.");
    } catch { setAssistantMsg("تعذر حفظ إعدادات Gemini."); } finally { setAssistantSaving(false); }
  }
      ...
      <main className="min-w-0 flex-1 overflow-x-hidden p-4 md:p-8">
        ...
        {tab === "settings" ? <div className="max-w-2xl space-y-6">
          <div className="rounded-xl border border-sand bg-white/85 p-5 md:p-7">
            <h3 className="flex items-center gap-2 font-display text-xl font-bold text-ink-900"><Bot size={20} className="text-steel-600" />إعدادات Gemini للمساعد الذكي</h3>
            <p className="mt-2 text-sm leading-7 text-muted">المساعد العام يستخدم Gemini لفهم الأسئلة ويعتمد على محتوى الموقع كاملًا قبل الإجابة. يفضّل استخدام GEMINI_API_KEY على Vercel لحماية المفتاح.</p>
            {assistant?.configured && !assistantEditing ? <div className="mt-5 rounded-lg border border-green-200 bg-green-50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold text-green-800">Gemini متصل</p><p className="mt-1 font-mono text-sm text-green-900 break-all">{assistant.maskedKey}</p><p className="mt-1 text-xs text-green-700">{assistant.source === "environment" ? "المصدر: Vercel Environment Variable" : "المصدر: إعدادات لوحة الإدارة"} — {assistant.model}</p></div><button onClick={() => setAssistantEditing(true)} className="btn btn-outline-ink !px-4 !py-2 !text-sm">استبدال المفتاح</button></div></div> : <div className="mt-5 space-y-4"><div><label className="field-label">Gemini API Key</label><input type="password" className="admin-input" value={assistantKey} onChange={(e) => setAssistantKey(e.target.value)} placeholder="AIza..." autoComplete="off" /></div><div><label className="field-label">Gemini Model</label><input className="admin-input" value={assistantModel} onChange={(e) => setAssistantModel(e.target.value)} placeholder="gemini-3.6-flash" /></div><div className="flex flex-wrap gap-2"><button onClick={saveAssistant} disabled={!assistantKey.trim() || assistantSaving} className="btn btn-ink !px-5 !py-2.5 !text-sm disabled:opacity-50">{assistantSaving ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}حفظ إعدادات Gemini</button>{assistantEditing ? <button onClick={() => { setAssistantEditing(false); setAssistantKey(""); }} className="btn btn-outline-ink !px-5 !py-2.5 !text-sm">إلغاء</button> : null}</div></div>}

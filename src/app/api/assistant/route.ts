...
function navigation(language: Language, message: string): SiteLink[] {
  ...
  // Colloquial Arabic + English/French intent matching. "أقدم/اقدم/أقدّم/أقدم ازاي"
  // are all treated as training-application intent rather than requiring an exact phrase.
  if (/تدريب|متدرب|تقديم.*تدريب|اقدم|أقدم|أقدّم|تقديم|intern|internship|training|apply.*(train|intern)|stage|postuler/i.test(q)) {
    add(language === "en" ? "Go to Training" : language === "fr" ? "Accéder aux stages" : "الانتقال إلى صفحة التدريب", "/training");
  }
  if (/خدمات|مجالات|تخصص|practice|services|specialit|domaines/i.test(q)) {
    add(language === "en" ? "View Services" : language === "fr" ? "Voir les services" : "عرض الخدمات ومجالات العمل", "/services");
  }
  if (/تواصل|اتصل|هاتف|موبايل|بريد|ايميل|إيميل|عنوان|contact|phone|email|address|coordonn/i.test(q)) {
    add(language === "en" ? "Contact the Firm" : language === "fr" ? "Contacter le cabinet" : "التواصل مع المكتب", "/contact");
  }
  if (/حسام|الدكتور|دكتور|من هو|نبذة|سيرة|about|doctor|profile|qui est|cabinet/i.test(q)) {
    add(language === "en" ? "About Dr. Hossam Loutfi" : language === "fr" ? "À propos de Dr. Hossam Loutfi" : "نبذة عن د. حسام لطفي", "/about");
  }
  if (/سؤال|اسئلة|أسئلة|faq|frequently|questions|questions fréquentes/i.test(q)) {
    add(language === "en" ? "Open FAQ" : language === "fr" ? "Ouvrir la FAQ" : "فتح الأسئلة الشائعة", "/faq");
  }
  if (/اكاديمي|أكاديمي|academia|academic|enseignement|teaching/i.test(q)) {
    add(language === "en" ? "Academic Profile" : language === "fr" ? "Profil académique" : "الملف الأكاديمي", "/academia");
  }
...
async function knowledge() {
  ...
  const text = JSON.stringify({
    ...
    services: services.map((s) => ({ title: s.title, summary: s.summary, paragraph: s.paragraph, audience: s.audience, items: s.items, note: s.note })),
...
function fallback(message: string, language: Language, data: any) {
  ...
  if (/تدريب|متدرب|تقديم.*تدريب|اقدم|أقدم|أقدّم|intern|internship|stage|training|postuler/.test(lower)) {
    return language === "en" ? `Training information: ${data.training?.intro ?? "Please use the training application section on the website."}` : language === "fr" ? `Informations sur le stage : ${data.training?.intro ?? "Veuillez utiliser la section de candidature du site."}` : `بخصوص التدريب: ${data.training?.intro ?? "يمكنك استخدام قسم التقديم للتدريب المنشور على الموقع."}`;
  }
...
export async function POST(req: Request) {
  try {
    ...
    const upstream = await fetch(endpoint, {
      ...
      body: JSON.stringify({ model, temperature: 0.1, messages: [{ role: "system", content: system }, ...messages.slice(-8).map((m: any) => ({ role: m.role === "assistant" ? "assistant" : "user", content: String(m.content ?? "") }))] }),
      cache: "no-store",
    });

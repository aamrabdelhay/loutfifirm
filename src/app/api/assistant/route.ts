...
function languageOf(text: string) {
  ...
  if (/\b(le|la|les|des|une|un|pour|avec|cabinet|stage|avocat)\b/i.test(text)) return "fr";
  return "en";
}

function refusal(language: string) {
...
function fallback(message: string, language: string, data: any) {
  const lower = message.toLowerCase();
  const c = data.contact ?? {};
  if (/تدريب|intern|stage|training/.test(lower)) return language === "en" ? `Training information: ${data.training?.intro ?? "Please use the training application section on the website."}` : language === "fr" ? `Informations sur le stage : ${data.training?.intro ?? "Veuillez utiliser la section de candidature du site."}` : `بخصوص التدريب: ${data.training?.intro ?? "يمكنك استخدام قسم التقديم للتدريب المنشور على الموقع."}`;
  if (/تواصل|contact|email|هاتف|phone|اتصل|adresse|عنوان/.test(lower)) return language === "en" ? `Contact: ${c.email ?? ""} ${c.mobile ?? ""}` : language === "fr" ? `Contact : ${c.email ?? ""} ${c.mobile ?? ""}` : `بيانات التواصل: ${c.email ?? ""} ${c.mobile ?? ""}`;
...
export async function POST(req: Request) {
  try {
    ...
    if (outsideScope) return Response.json({ ok: true, answer: refusal(language), source: "scope" });
    ...
    if (!upstream.ok) return Response.json({ ok: true, answer: fallback(latest, language, data), source: "site-fallback" });
    const json = await upstream.json();
    const answer = json?.choices?.[0]?.message?.content;
    if (!answer) return Response.json({ ok: true, answer: fallback(latest, language, data), source: "site-fallback" });
    return Response.json({ ok: true, answer, source: "ai" });
  } catch {
    return Response.json({ ok: true, answer: "تعذر الاتصال بالمساعد الذكي حاليًا. يُرجى التواصل مع المكتب مباشرة عبر بيانات الاتصال المنشورة على الموقع.", source: "error-fallback" });
  }
}
